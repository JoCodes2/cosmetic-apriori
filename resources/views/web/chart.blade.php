...
@section('content')
    ...
    <div class="flex flex-col lg:flex-row lg:space-x-8">
        ...
        <div class="w-full lg:w-2/3 mb-8 lg:mb-0">
            ...
            <div class="mt-8">
                <h2 class="text-xl font-bold mb-4">Rekomendasi Produk Lainnya Untuk Anda</h2>
                <div id="recommendation-container" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6">
                    <!-- Produk rekomendasi akan ditampilkan di sini -->
                </div>
            </div>
        </div>
...
$(document).ready(function () {
    ...
    function loadRecommendations() {
        let cart = JSON.parse(localStorage.getItem("cart")) || [];
        if (cart.length === 0) {
            $("#recommendation-container").html(`<p class="text-gray-500">Tidak ada rekomendasi.</p>`);
            return;
        }

        let productIds = cart.map(item => item.id_product);

        $.ajax({
            url: "/v1/order/recommendations",
            type: "POST",
            data: { product_ids: productIds },
            success: function (response) {
                console.log(response);

                let recommendations = response.recommended_products;
                let container = $("#recommendation-container");
                container.empty();

                if (recommendations.length === 0) {
                    container.html(`<p class="text-gray-500">Tidak ada rekomendasi untuk produk ini.</p>`);
                    return;
                }
                ...
                recommendations.forEach(product => {
                    let productCard = `
                        <div class="bg-white shadow rounded-lg overflow-hidden p-2">
                            <img class="w-full h-32 object-cover rounded-md" src="/uploads/img-product/${product.image}" alt="${product.name}">
                            <div class="p-2">
                                <h2 class="text-sm font-semibold truncate">${product.name}</h2>
                                <p class="text-xs text-gray-600">Rp ${product.price.toLocaleString()}</p>
                                <button class="add-to-cart mt-2 w-full bg-pink-400 border border-gray-300 text-white py-1 rounded text-xs flex items-center justify-center space-x-1"
                                    data-id="${product.id}"
